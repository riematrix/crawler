...
(function() {
  ...
  root.DocxGen = DocxGen = (function() {
    ...
    DocxGen.prototype.addOleObjectRels = function(objectName, data) {
      var extension, file, newTag, relationships;
      if (this.zip.files["word/embeddings/" + objectName] != null) {
        throw 'file already exists';
      }
      this.maxRid++;
      file = {
        'name': "word/embeddings/" + objectName,
        'data': data,
        'options': {
          base64: false,
          binary: true,
      ...
      this.zip.file(file.name, file.data, file.options);
      extension = objectName.replace(/[^.]+\.([^.]+)/, '$1');
      this.addExtensionRels("application/vnd.openxmlformats-officedocument.oleObject", "bin");
      relationships = this.xmlDoc.getElementsByTagName("Relationships")[0];
      newTag = this.xmlDoc.createElement('Relationship');
      newTag.namespaceURI = null;
      newTag.setAttribute('Id', "rId" + this.maxRid);
      newTag.setAttribute('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/oleObject');
      newTag.setAttribute('Target', "embeddings/" + objectName);
      relationships.appendChild(newTag);
      this.zip.files["word/_rels/document.xml.rels"].data = DocUtils.encode_utf8(DocUtils.xml2Str(this.xmlDoc));
        console.log("===========end addOleObjectRel:"+objectName)
      return this.maxRid;
    };
  ...
  var ObjectReplacer = (function() {
    function ObjectReplacer(xmlTemplater) {
      this.xmlTemplater = xmlTemplater;
      this.objectMatches = [];
    }

    ObjectReplacer.prototype.findObjects = function() {
      return this.objectMatches = DocUtils.preg_match_all(/<w:object[^>]*>.*?<\/w:object>/g, this.xmlTemplater.content);
    };

    ObjectReplacer.prototype.replaceObjects = function() {
      var objectTag, objData, objName, match, newId, oldFile, qr, rId, replacement, shape, tagrId, u, xmlObj, _i, _len, _ref, _results,
        _this = this;
      console.log('replacing Objects ...');
      qr = [];
      _ref = this.objectMatches;
      _results = [];
      console.log(_ref)
      for (u = _i = 0, _len = _ref.length; _i < _len; u = ++_i) {
        match = _ref[u];
        xmlObj = DocUtils.Str2xml('<?xml version="1.0" ?><w:document mc:Ignorable="w14 wp14" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:w10="urn:schemas-microsoft-com:office:word" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" xmlns:wne="http://schemas.microsoft.com/office/word/2006/wordml" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:wp14="http://schemas.microsoft.com/office/word/2010/wordprocessingDrawing" xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" xmlns:wpg="http://schemas.microsoft.com/office/word/2010/wordprocessingGroup" xmlns:wpi="http://schemas.microsoft.com/office/word/2010/wordprocessingInk" xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape">' + match[0] + '</w:document>');
        if (this.xmlTemplater.currentScope["ole"] != null) {
          if (this.xmlTemplater.currentScope["ole"][u] != null) {
            objName = this.xmlTemplater.currentScope["ole"][u].name;
            objData = this.xmlTemplater.currentScope["ole"][u].data;
            if (this.xmlTemplater.DocxGen == null) {
              throw 'DocxGen not defined';
            }
            newId = this.xmlTemplater.DocxGen.addOleObjectRels(objName, objData);
            shape = xmlObj.getElementsByTagNameNS('*', 'shape')[0];
            if (shape === void 0) {
              console.log('tag not defined, trying alternate method');
              shape = xmlObj.getElementsByTagName('v:shape')[0];
            }
            if (shape !== void 0) {
              this.xmlTemplater.shapeId++;
              shape.setAttribute('id', "_x0000_i103"+newId);
              shape.setAttribute('name', "" + objName);
              tagrId = xmlObj.getElementsByTagNameNS('*', 'OLEObject')[0];
              if (tagrId === void 0) {
                console.log('tagRid not defined, trying alternate method');
                tagrId = xmlObj.getElementsByTagName("o:OLEObject")[0];
              }
              if (tagrId !== void 0) {
                tagrId.setAttribute('r:id', "rId" + newId);
                tagrId.setAttribute('ObjectID','_144863080'+newId);
                tagrId.setAttribute('ShapeID',shape.getAttribute('id'));
                objectTag = xmlObj.getElementsByTagNameNS('*', 'object')[0];
                  console.log(objectTag);
                if (objectTag === void 0) {
                  console.log('objecttag not defined, trying alternate method');
                  objectTag = xmlObj.getElementsByTagName('w:object')[0];
                }
                _results.push(this.xmlTemplater.content = this.xmlTemplater.content.replace(match[0], DocUtils.xml2Str(objectTag)));
              } else {
                _results.push(void 0);
              }
            } else {
  ...
  root.ObjectReplacer = ObjectReplacer;//TODO
  ...
  XmlTemplater = XmlTemplater = (function() {
    ...
    */
    ...
    XmlTemplater.prototype.applyTemplateVars = function() {
      ...
      var objectReplacer = new ObjectReplacer(this);//TODO
      objectReplacer.findObjects();
      objectReplacer.replaceObjects();
      return this;
    };
